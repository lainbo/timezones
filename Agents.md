...
5. 当地时间可能落入夏令时空缺或重叠。`resolveLocalTime` 返回 `gap`、`overlap` 或 `exact`：空缺需要提示用户改选，建议时间跨日时附带日期；重叠需要用户选择具体的一次发生时刻及其偏移，打开时间弹窗时预选卡片当前显示的那一次。空缺和重叠也可能来自标准时间调整，提示文案按“时钟拨快/回拨”描述。
6. 当地一天的时长由相邻两个当地日界计算，不能固定为 24 小时。刻度应支持夏令时日期的 23 或 25 小时，以及半小时、四十五分钟偏移。
7. 日期选择范围为 1900 至 2100 年。日期切换按当地日历日计算；结果的当地日期时间与预期不同（落入空缺）时，顺延到有效时间并提示。
...
- 卡片右上角的移除按钮用 CSS 控制显示：鼠标悬停卡片，或卡片内有键盘焦点（`:focus-visible`）时显示；窄屏和无悬停能力的设备常驻显示。鼠标点击留下的焦点不会让它保持显示。
...
- favicon 固定使用米白底（`#f6f6f2`）、深绿色（`#39483b`）与橙色（`#c7704e`）圆点。
...
- 卡片在当地时间跨越 06:00 或 18:00 时，颜色在 500ms 内渐变。卡片内使用且会被 `.dark` 改写的颜色变量在 `src/index.css` 中用 `@property` 注册为 `<color>`，由卡片上的 `transition-day-night` 过渡，内部元素继承过渡中的值；钟面按昼夜换用不同变量，自带同样时长的过渡。卡片内新增随昼夜变化的颜色变量时，同时注册并加入过渡列表。星空图层随昼夜在 500ms 内淡入淡出；亮星的闪烁只改变透明度，仅在夜晚运行。
...
- 尊重系统“减少动态效果”偏好，主题切换、卡片昼夜颜色和星空在此情况下直接更新，亮星不闪烁。
