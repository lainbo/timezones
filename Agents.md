...
- 主题以黑白灰为主，警告与删除状态保留语义提示色。夜晚卡片局部使用 `.dark` 主题变量，采用炭黑底与浅色文字；白天卡片随页面主题调整明度。深色模式下白天标签的太阳图标使用 `day-icon` 浅金黄色。昼夜状态始终显示太阳／月亮图标与文字标签，按当地时间 06:00–18:59 / 19:00–05:59 划分，刻度分别使用浅灰／深灰。
